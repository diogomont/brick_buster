...
#define LEFT_BOUND 47
#define RIGHT_BOUND 194
#define LOWER_BOUND 152
#define UPPER_BOUND 9
#define BRICKGRID_SIZE 54
#define BRICK_X(c) LEFT_BOUND + 1 + (BLOCK_BLUE_WIDTH * c)
#define BRICK_Y(r) 20 + (BLOCK_BLUE_HEIGHT * r)
#define BRICK_COL(x) ((x-48)/16)
#define BRICK_ROW(y) ((y-20)/8)
...
void new_life();
void init_brick_array();
void launch_ball();
void wait_for_vsync();
void dma_cpy(u16* dst, const volatile u16* src, u32 count, u8 fixed_src);
void draw(s16 x, s16 y, const volatile u16* image, u8 width, u8 height);
void draw_background(volatile s16 x, volatile s16 y, const volatile u16* image, u8 width, u8 height);
void draw_brick_row(s8 row_number);
s8 player_direction();
s8 check_paddle_collision();
s16 new_ball_direction();
void draw_no_corners(volatile s16 x, volatile s16 y, const volatile u16* image, u8 width, u8 height);
u8 kill_brick(s8 index);
